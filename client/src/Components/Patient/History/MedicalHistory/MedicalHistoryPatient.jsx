...
import MedicalDiagnoseHistoryPatient from './MedicalDiagnoseHistoryPatient';
...
export default function MedicalHistoryPatient() {
  ...
  let crum = ['Home','My History', 'Medical History'];
  ...
  async function get(){
    var res
    try{
       res=await axios.get(`${path}history/patient/particularmedicalhistory/${user?._id}/${series}`)
  ...
  },[path,series,user._id])

      async function getdat(fileUrl){
      var x = fileUrl
      setfile({path: 'Hello', file: null || x})
    }
  ...
  const getFilePluginInstance = getFilePlugin('Hello.jsx');
   ...
   return (
    ...
    (err)?(<div className='text-center '><p ><b>No History</b></p></div>):(<>
    <ButtonGroup overrides={{ Root: { style: ({ $theme }) => ({ display: 'flex', justifyContent: 'space-between', width: '80%', margin: 'auto',}) } }}>
      ...
      {initial && <Button endEnhancer={<SkipForward/>} onClick={()=>{series < initial.maxLen && setSeries(p=>p+1); setfile({file:null})}}>Next</Button>}
    </ButtonGroup>
    {initial && <MedicalDiagnoseHistoryPatient initial={initial} />}
